...
namespace ins {  
    ...
    Vec3 calculate(float accelometer[3], float gyroscope[4], unsigned long samplePeriod, Previous &previous ) {
        float dt = (float)samplePeriod / 1000.0;
        Quat orientation = orientation = Quat(gyroscope[0], gyroscope[1], gyroscope[2], gyroscope[3]);
